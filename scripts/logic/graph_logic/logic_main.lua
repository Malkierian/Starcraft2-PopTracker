...
starcraft_2_location = {}
starcraft_2_location.__index = starcraft_2_location
...
function starcraft_2_location.new(name)
    local self = setmetatable({}, starcraft_2_location)
    if name then
...
function starcraft_2_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = starcraft_2_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function starcraft_2_location:connect_two_ways(exit, rule)
...
function starcraft_2_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function starcraft_2_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function starcraft_2_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function starcraft_2_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function starcraft_2_location:accessibility()
...
-- 
function starcraft_2_location:discover(accessibility, keys)
...
entry_point = starcraft_2_location.new("entry_point")
